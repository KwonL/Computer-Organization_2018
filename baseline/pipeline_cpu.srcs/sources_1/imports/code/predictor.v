    ...
    always @ (posedge clk) begin
        if (send_addr_sig) begin
            ...
            // if tag match and prediction wrong, then rewrite BTB and -- state
                ...
                // compare state to 0 to prevent overflow
                if (state[index_prev] > 0) state[index_prev] <= state[index_prev] - 1;
                ...
                if (Actual_address != PC_prev + 1) begin
                    ...
                    if (state[index_prev] <= 1) incorrect <= incorrect + 1;
                    else correct <= correct + 1;
                end
            end
            // if tag match and prediction correct, then ++ state
            else if (tag[index_prev] == tag_prev && (BTB[index_prev] == Actual_address)) begin
                // compare state to 3 to prevent overflow
                if (state[index_prev] < 3) state[index_prev] <= state[index_prev] + 1;

                // only count for predicted result
                if (state[index_prev] >= 2) correct <= correct + 1;
...
module Branch_predictor_always (
    input clk,
    input prev_taken,
